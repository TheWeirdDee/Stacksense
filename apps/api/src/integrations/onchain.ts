  ...
  try {
    while (offset < 500) {
      ...
      for (const tx of results) {
        ...
        if (sender) callerSet.add(sender);
      }
 ...
 */
export async function getWalletDeployments(address: string): Promise<number> {
  try {
    let offset = 0;
    const limit = 50;
    let deployCount = 0;
 ...
 */
export async function getWalletOnchainStats(
  ownerAddress: string,
  contracts: Array<{ address: string; name: string }>
): Promise<WalletOnchainStats> {
  ...
  const callerUnion = new Set<string>();
