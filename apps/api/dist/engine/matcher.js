...
function matchesContractId(ruleContractId, txContractId) {
    if (!ruleContractId)
        return true;
    if (!txContractId)
        return false;
    if (ruleContractId.toLowerCase() === txContractId.toLowerCase())
        return true;
    const ruleName = ruleContractId.split('.')[1]?.toLowerCase();
    const txName = txContractId.split('.')[1]?.toLowerCase();
    if (ruleName && txName && ruleName === txName)
        return true;
    return false;
}
export async function matchTransaction(tx) {
    ...
    if (tx_type === 'token_transfer') {
        ...
        if (tx.post_conditions && Array.isArray(tx.post_conditions)) {
            const senderPostConditions = tx.post_conditions.filter((pc) => pc.condition_code === 'sent_equal_to' || pc.condition_code === 'sent_greater_than_or_equal_to');
            let totalMicroSTX = 0;
        ...
        if (stx_amount === 0 && tx.stx_transfers && Array.isArray(tx.stx_transfers)) {
            const totalInternal = tx.stx_transfers
                .filter((t) => t.sender === tx.sender_address)
    ...
    for (const rule of rules) {
        if (rule.tx_type !== tx_type)
            continue;
        if (!matchesContractId(rule.contract_id, contract_id))
            continue;
        if (rule.function_name && rule.function_name !== function_name)
            continue;
        if (rule.min_stx !== null && stx_amount < rule.min_stx)
            continue;
        if (rule.max_stx !== null && stx_amount > rule.max_stx)
            continue;
        matchedRule = rule;
        break;
    }
