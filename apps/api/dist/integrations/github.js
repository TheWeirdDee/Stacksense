...
export async function trackGitHubActivity(username) {
    try {
...
export async function getGitHubStats(username) {
    try {
        ...
        const searchQuery = STACKS_KEYWORDS.map((kw) => `${kw} author:${username}`).join(' OR ');
...
// Register developer
export async function registerDeveloper(username, stacksAddress) {
    try {
        // Verify GitHub user exists
        await axios.get(`${GITHUB_API_BASE}/users/${username}`, { timeout: 5000 });
        // Store mapping
        await redisClient.set(`dev:github:${username}`, JSON.stringify({
            stacksAddress,
...
export async function getDeveloperStats(stacksAddress) {
...
export async function getRepoStats(owner, repo) {
    try {
        ...
        const totalCommits = Array.isArray(contributorsData)
            ? contributorsData.reduce((sum, c) => sum + (c.contributions || 0), 0)
            : commitsData.length;
        ...
        const isEcoRepo = STACKS_ECOSYSTEM_REPOS.some((r) => r.toLowerCase() === repoFullName);
        if (isEcoRepo)
            ecosystemCommits = totalCommits;
        return {
            owner,
            repo,
            totalCommits,
