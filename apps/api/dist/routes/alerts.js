...
router.post('/create', async (req, res) => {
    try {
        ...
        await redisClient.set(`webhook:${webhookId}`, JSON.stringify(alert), { EX: 31536000 } // 1 year
        );
...
export async function broadcastToWebhooks(event) {
    try {
        ...
        for (const key of keys) {
            ...
            if (!webhookData)
                continue;
            const alert = JSON.parse(webhookData);
            if (!alert.active)
                continue;
...
function checkFilters(event, filters) {
...
async function logWebhookDelivery(subscriberAddress) {
...
export async function getWebhookUsage(subscriberAddress) {
