...
router.post('/api-keys/generate', async (req, res) => {
    try {
        ...
        await redisClient.set(`api-key:${keyHash}`, JSON.stringify({
            tier,
            subscriberAddress,
...
router.post('/api-key/regenerate', async (req, res) => {
    try {
        ...
        await redisClient.set(`api-key:${newKeyHash}`, JSON.stringify({
            ...parsed,
            createdAt: new Date().toISOString(),
            requestsUsed: 0,
        }), { EX: 2592000 });
...
export async function checkApiQuota(apiKey) {
    if (!apiKey)
        return true; // Allow free tier
    const keyHash = crypto.createHash('sha256').update(apiKey).digest('hex');
    const keyData = await redisClient.get(`api-key:${keyHash}`);
    if (!keyData) {
