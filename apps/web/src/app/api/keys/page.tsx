  ...
  return (
    <div style={{ background: 'var(--bg-base)', minHeight: '100vh', padding: '20px 0 60px' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 20px' }}>
        <Link href="/api" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--text-secondary)', fontSize: 13, textDecoration: 'none', marginBottom: 24 }}>
          ← Back to API Hub
        </Link>
        ...
        {!connected ? (
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--bg-border)', borderRadius: 12, padding: '48px 32px', textAlign: 'center' }}>
            <Key size={36} style={{ color: 'var(--text-muted)', marginBottom: 16 }} />
            ...
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', maxWidth: 400, margin: '0 auto 24px' }}>
              Your API key is tied to your Stacks wallet address. Connect to view, copy, or rotate your key and monitor quota usage against your active plan.
            </p>
            <button
              onClick={connect}
              ...
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--bg-border)', borderRadius: 8, padding: 24 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'var(--bg-base)', border: '1px solid var(--bg-border)', borderRadius: 6, padding: '12px 16px', marginBottom: 16 }}>
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: '1px solid var(--bg-border)', borderRadius: 4, padding: '6px 8px', cursor: 'pointer', color: copied ? '#22C55E' : 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0, transition: 'color 0.2s' }}
                  >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>
                      {apiKeyInfo ? `${apiKeyInfo.requestsUsed.toLocaleString()} / ${apiKeyInfo.requestsLimit.toLocaleString()}` : '—'}
                    </p>
                  </div>
                </div>
